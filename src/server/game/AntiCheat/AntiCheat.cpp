...
{
    sLog->outDebug(LOG_FILTER_NONE, "AntiCheat: CheckNull called");
    return true;
};
...
bool AntiCheat::_DoAntiCheatCheck(AntiCheatCheck checktype)
{
    ...
    if (_check->active && CheckTimer(checktype) && CheckNeeded(checktype))
    {
        ...
        {
            if (getMSTimeDiff(m_lastalarmtime[checktype], getMSTime()) > sWorld->getIntConfig(CONFIG_UINT32_ANTICHEAT_ACTION_DELAY)
...
void AntiCheat::DoAntiCheatAction(AntiCheatCheck checkType, std::string reason)
{
    ...
    if (getMSTime() - m_lastactiontime[checkType] >= sWorld->getIntConfig(CONFIG_UINT32_ANTICHEAT_ACTION_DELAY) * 1000)
    {
    ...
    if (config->actionType[0] != ANTICHEAT_ACTION_NULL)
    {
        ...
        if (!sWorld->getBoolConfig(CONFIG_BOOL_ANTICHEAT_DEBUG))
        {
...
{
    if (!sWorld->getBoolConfig(CONFIG_BOOL_ANTICHEAT_ENABLE)
        || !GetPlayer()->IsInWorld()
        || GetPlayer()->IsBeingTeleported()
        || (uint32)GetPlayer()->GetSession()->GetSecurity() > sWorld->getIntConfig(CONFIG_UINT32_ANTICHEAT_GMLEVEL))
        return false;
    ...
    switch( checktype)
    {
        ...
        case CHECK_MOVEMENT_TP2PLANE:
            ...
            if (m_currentmovementInfo->HasMovementFlag(MovementFlags(MOVEMENTFLAG_SWIMMING | MOVEMENTFLAG_CAN_FLY | MOVEMENTFLAG_FLYING)))
                return false;
            if (GetMover()->HasAura(60068) && GetMover()->GetMap()->IsUnderWater(m_currentmovementInfo->pos.GetPositionX(), m_currentmovementInfo->pos.GetPositionY(), m_currentmovementInfo->pos.GetPositionZ()-5.0f))
                return false;
            break;
